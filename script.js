...
try {
  await fetchFakeData().then ((data) => console.log(data));
} catch(e) {
  console.error(e);
}
}
...
async function logRandomNumberAfterSeconds() {
  try {
    ...
    await getRandomNumberAfterSeconds(randomNumber).then ((value) => console.log(value));
  } catch(e) {
    console.error(e);
  }
}
...
let i=0;
while (true) {yield new Promise((resolve) => {
  setTimeout(() => {
    resolve(i);
    i++}, 1000);  
})
...
async function displayData() {
  const genGatherData = gatherData();
 
  console.log(await genGatherData.next());
  console.log(await genGatherData.next());
  console.log(await genGatherData.next());
}
...
const genCountDown = countdownGenerator(5);
...
let nextValue = genCountDown.next();
  while (nextValue.done === false) {
   console.log(nextValue.value);
    nextValue = genCountDown.next();
  }
